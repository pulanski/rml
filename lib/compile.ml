...
let compile_to_target ir target base_output_file =
  match target with
  | "c" ->
      (* Generate header file *)
      let header_code = emit_c_header ir in
      ...
      let source_code = emit_c_source ir in
...
let compile input_file output_config =
  ...
  if output_config.output_c then
    time_phase "C Codegen" (fun () -> compile_to_target ir "c" (Filename.chop_suffix input_file ".rs"));
